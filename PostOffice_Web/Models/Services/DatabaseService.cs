...
public class DatabaseService(PostOfficeContext postOfficeContext)
{
	...
	public void SourceInitialization()
	{
		...
		postOfficeContext.SaveChanges();
		postOfficeContext.Sections.AddRange(DataGeneration.SelectionGeneration(8, GetPeopleTable().Where(item => item.Role == Roles.Postman).ToList()));
		postOfficeContext.SaveChanges();
	...
	public async Task<List<Section>> GetSelectionsTableAsync() =>
		await postOfficeContext
		.Sections
		.ToListAsync();
